...
def fidelity_for_setting(
    ...
    if order_mode == "high-hamming-first":
        ordered_bitstrings, encoded_phases, amplitudes = (
            high_hamming_first_order(
                bitstrings,
                encoded_phases,
                amplitudes,
        ...
        if len(ordered_bitstrings) != len(encoded_phases):
            raise RuntimeError("Mode ordering returned inconsistent data.")
...
def final_comparison_curves(
    ...
    for qubit_count in qubit_counts:
        ...
        if len(bitstrings) != len(weights):
            raise RuntimeError("Mode weighting returned inconsistent data.")
